...
from ..message_validation import is_emoji_only, is_gibberish, enforce_max_words
	...
	def post(self, request, *args, **kwargs):
		...
		try:
			...
			if is_emoji_only(message):
				corrective_response = enforce_max_words("I noticed you reacted with an emoji. Could you please send your message in words so I can help you better?", max_words=15)
			...
			if is_gibberish(message):
				corrective_response = enforce_max_words("I couldn't understand that message. Could you please rephrase your question more clearly?", max_words=15)
				ai_message = ChatMessage.objects.create(session=chat_session, user=user, content=corrective_response, is_user_message=False, response_time_ms=0, token_count=len(corrective_response.split()))
				chat_session.save()
				return Response({"response": corrective_response, "user_message": message, "session_id": str(chat_session.id), "message_id": str(ai_message.id), "response_time_ms": 0})
